...
setup:
	...
	cd mobatech-backend && GOTOOLCHAIN=auto go mod tidy
...
db-reset:
	...
	@cd mobatech-backend && GOTOOLCHAIN=auto go run cmd/seed/main.go
...
# ==============================================================================
run-backend:
	cd mobatech-backend && (which air >/dev/null 2>&1 && air || test -x $(HOME)/go/bin/air && $(HOME)/go/bin/air || GOTOOLCHAIN=auto go run main.go)
...
build-apk:
	@echo "Building Universal Release APK (Full Support)..."
