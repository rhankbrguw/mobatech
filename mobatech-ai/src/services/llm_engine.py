...
backend_env_path = os.path.join(
    os.path.dirname(__file__), const.LLM_BACKEND_ENV_PATH_REL
)
...
class GenerativeEngine:
    ...
    def generate_response(self, query: str, contexts: list[str]) -> str:
        ...
        prompt = const.PROMPT_TEMPLATE.format(
            current_time_str=current_time_str, context_str=context_str, query=query
        )

        try:
            response = self.client.models.generate_content(
                model=const.GEMINI_MODEL_NAME, contents=prompt
            )
