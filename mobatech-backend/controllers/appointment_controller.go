...
func (c *AppointmentController) GetAllAppointments(ctx *gin.Context) {
	...
	if err != nil {
		_ = ctx.Error(utils.NewInternalError(err.Error()))
		return
	}
...
func (c *AppointmentController) GetUserAppointments(ctx *gin.Context) {
	...
	if !exists {
		_ = ctx.Error(utils.NewAppError(utils.ErrUnauthenticated, http.StatusUnauthorized, constants.MsgUnauthorized, nil))
		return
	}
	userID := uint(userIDFloat.(float64))
...
func (c *AppointmentController) BookAppointment(ctx *gin.Context) {
	...
	services.NotifyAppointmentBooked(config.DB, appointment)

	ctx.JSON(http.StatusOK, utils.BuildSuccess("OK", "Success", appointment))
}

func parseCancelReason(ctx *gin.Context) string {
	var req struct {
...
func (c *AppointmentController) CancelAppointment(ctx *gin.Context) {
	...
	reason := parseCancelReason(ctx)
	appointment, err := c.appointmentService.CancelAppointment(ctx.Request.Context(), uint(id), uint(userIDFloat.(float64)), "patient", reason)
	if err != nil {
		_ = ctx.Error(utils.FormatValidationError(err))
		return
	}

	services.NotifyAppointmentCancelled(config.DB, appointment, "patient", reason)
