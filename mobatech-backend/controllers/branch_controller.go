...
func parseBranchQueryParams(c *gin.Context) (string, string, int, int, int, error) {
	search := c.Query(constants.QUERY_PARAM_SEARCH)
	filter := c.Query(constants.QUERY_PARAM_FILTER)

	page, err := strconv.Atoi(c.DefaultQuery(constants.QUERY_PARAM_PAGE, constants.PAGINATION_DEFAULT_PAGE))
	if err != nil {
		return "", "", 0, 0, 0, utils.NewValidationError("Invalid page parameter")
	}
	limit, err := strconv.Atoi(c.DefaultQuery(constants.QUERY_PARAM_LIMIT, constants.PAGINATION_DEFAULT_LIMIT))
	if err != nil {
...
func (ctrl *BranchController) GetBranches(c *gin.Context) {
	search, filter, page, limit, offset, err := parseBranchQueryParams(c)
	if err != nil {
		_ = c.Error(err)
		return
	}
