...
func (c *DoctorController) CreateDoctor(ctx *gin.Context) {
	...
	if doctor.Email != "" {
		if err := utils.ValidateEmail(doctor.Email); err != nil {
			ctx.Error(err)
			return
		}
		if err := utils.ValidatePassword(doctor.Password); err != nil {
			ctx.Error(err)
			return
		}
	}
...
func bindDoctorUpdateInput(ctx *gin.Context) (*models.Doctor, error) {
...
func (c *DoctorController) UpdateDoctor(ctx *gin.Context) {
	...
	doc, err := c.doctorService.GetDoctorByID(ctx.Request.Context(), uint(id))
	if err != nil {
		ctx.Error(utils.NewAppError(utils.ErrNotFound, http.StatusNotFound, constants.MsgDoctorNotFound, nil))
		return
	}
	if !checkDoctorOwnership(ctx, doc) {
		ctx.AbortWithStatusJSON(http.StatusForbidden, utils.BuildError(utils.ErrUnauthorized, "Hanya dapat mengubah profil Anda sendiri", nil))
		return
	}
	input, err := bindDoctorUpdateInput(ctx)
	if err != nil {
		ctx.Error(err)
		return
	}
