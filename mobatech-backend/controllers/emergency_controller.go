...
func (c *EmergencyController) SubmitRequest(ctx *gin.Context) {
	...
	autoDispatchEmergency(c.service, req.ID)
...
func autoDispatchEmergency(service services.EmergencyService, emergencyID uint) {
	go func() {
		defer func() {
			if r := recover(); r != nil {
		...
		_ = service.UpdateStatus(context.Background(), emergencyID, "Dispatched")
	}()
}
