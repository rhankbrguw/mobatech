...
func (c *PharmacyController) AdminCreatePrescription(ctx *gin.Context) {
	...
	ctx.JSON(http.StatusCreated, utils.BuildSuccess("CREATED", constants.MsgResourceCreated, p))
}
...
func (c *PharmacyController) AdminGetAllPrescriptions(ctx *gin.Context) {
	...
	prescriptions, totalCount, err := c.service.GetAllPrescriptions(ctx.Request.Context(), search, filter, limit, offset)
	if err != nil {
		ctx.Error(utils.NewInternalError(err.Error()))
		return
	}
