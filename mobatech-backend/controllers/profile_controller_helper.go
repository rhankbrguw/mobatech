...
func parseProfileNumerics(ctx *gin.Context) (int, int, error) {
	height, err := strconv.Atoi(ctx.PostForm("height"))
	if err != nil {
		return 0, 0, utils.NewValidationError("Invalid height parameter")
	}
	weight, err := strconv.Atoi(ctx.PostForm("weight"))
	if err != nil {
		return 0, 0, utils.NewValidationError("Invalid weight parameter")
	}
	return height, weight, nil
}

func validateProfileFields(name, phone string) error {
	if name != "" {
		if err := utils.ValidateName(name, "Nama lengkap"); err != nil {
			return err
		}
	}
	if phone != "" {
...
func parseProfileUpdateRequest(ctx *gin.Context) (*profileUpdateRequest, error) {
	height, weight, err := parseProfileNumerics(ctx)
	if err != nil {
		return nil, err
	}
	fullName := ctx.PostForm("full_name")
	phone := ctx.PostForm("phone_number")
	if err := validateProfileFields(fullName, phone); err != nil {
		return nil, err
	}
