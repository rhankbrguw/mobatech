...
func (c *ScheduleController) GetAllSchedules(ctx *gin.Context) {
	...
	schedules, totalCount, err := c.scheduleService.GetUpcomingSchedules(ctx.Request.Context(), limit, offset)
	if err != nil {
		ctx.Error(utils.NewInternalError(err.Error()))
		return
	}
	ctx.JSON(http.StatusOK, utils.BuildPaginatedSuccess("Success", schedules, page, limit, totalCount))
}
