...
type AppointmentRepository interface {
	...
	HasMedicalResult(ctx context.Context, appointmentID uint) (bool, error)
}
...
func (r *appointmentRepository) FindAll(ctx context.Context, search string, filter string, userID uint, role string, limit, offset int) ([]models.Appointment, int64, error) {
	...
	r.populateMedicalResultFlags(appointments)
...
func (r *appointmentRepository) FindByID(ctx context.Context, id uint) (*models.Appointment, error) {
	...
	hasResult, _ := r.HasMedicalResult(ctx, appointment.ID)
	appointment.HasMedicalResult = hasResult
...
func (r *appointmentRepository) HasMedicalResult(ctx context.Context, appointmentID uint) (bool, error) {
	var count int64
	err := r.db.Model(&models.MedicalResult{}).Where("appointment_id = ?", appointmentID).Count(&count).Error
	return count > 0, err
}

func (r *appointmentRepository) populateMedicalResultFlags(appointments []models.Appointment) {
	if len(appointments) == 0 {
		return
	}
	ids := make([]uint, len(appointments))
	for i, a := range appointments {
		ids[i] = a.ID
	}
	var resIDs []uint
	r.db.Model(&models.MedicalResult{}).Where("appointment_id IN ?", ids).Pluck("appointment_id", &resIDs)
	resMap := make(map[uint]bool)
	for _, id := range resIDs {
		resMap[id] = true
	}
	for i := range appointments {
		appointments[i].HasMedicalResult = resMap[appointments[i].ID]
	}
}
