...
type PharmacyRepository interface {
	...
	GetAllPrescriptions(ctx context.Context, search string, filter string, limit int, offset int) ([]models.Prescription, int64, error)
	...
	GetOrdersByUserID(ctx context.Context, userID uint, limit int, offset int) ([]models.PharmacyOrder, int64, error)
...
func (r *pharmacyRepository) UpdateMedicineStock(ctx context.Context, id uint, quantityChange int) error {
	return r.db.Transaction(func(tx *gorm.DB) error {
		...
		if med.Stock+quantityChange < 0 {
			return fmt.Errorf("pharmacyRepository.UpdateMedicineStock: %w", constants.ErrInsufficientStock)
		}
