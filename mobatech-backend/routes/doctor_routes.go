...
func SetupDoctorRoutes(router *gin.Engine, db *gorm.DB, ragClient services.RAGClient) {
	...
	h := &doctorRouteHandlers{
		...
		schCtrl:  controllers.NewScheduleController(services.NewScheduleService(scheduleRepo, ragClient)),
...
func setupAdminDoctorRoutes(api *gin.RouterGroup, h *doctorRouteHandlers) {
	...
	admin.PUT(constants.RouteDoctorsParamId, middleware.RequireRole("admin", "doctor"), h.docCtrl.UpdateDoctor)
	...
	admin.POST(constants.RouteAppointmentsParamIdApprove, middleware.RequireRole("admin", "doctor"), h.apptCtrl.ApproveAppointment)
