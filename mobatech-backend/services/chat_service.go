...
func NewChatService(repo repositories.ChatRepository, ragClient RAGClient) ChatService {
	if ragClient == nil {
...
func (s *chatService) StreamChat(ctx context.Context, sessionID uint, userMessage string, outChan chan<- string, errChan chan<- error) {
	...
	if err != nil {
		errChan <- fmt.Errorf("failed to get history: %w", err)
		return
	}

	if len(historyMsg) == 1 {
		go func() { _ = s.asyncGenerateTitle(context.Background(), sessionID, userMessage) }()
	}
...
func (s *chatService) asyncGenerateTitle(ctx context.Context, sessionID uint, firstMessage string) error {
