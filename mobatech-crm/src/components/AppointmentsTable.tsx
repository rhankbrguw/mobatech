...
interface AppointmentsTableProps {
  ...
  onComplete: (id: number, hasMedicalResult?: boolean, item?: Appointment) => void;
...
export function AppointmentsTable({ items, loading, processingId, onApprove, onCancel, onComplete, onViewDetails, userRole }: AppointmentsTableProps) {
  ...
  const getStatusBadge = (status: string, hasMedicalResult?: boolean) => {
    ...
    switch (status) {
      case "approved":
        variant = "success";
        label = hasMedicalResult ? "Disetujui (RME Siap)" : "Disetujui (RME Belum Diisi)";
        break;
      case "cancelled":
        variant = "error";
        label = "Dibatalkan";
        ...
        <tbody>
          {items.map((item) => (
            <tr key={item.id} className="hover:bg-overlay-dark dark:hover:bg-overlay-light transition-colors group">
              ...
              <td className={`${TD_CLASS} text-center font-medium text-foreground`}>
                {getStatusBadge(item.status, item.has_medical_result)}
              </td>
              <td className={`${TD_CLASS} text-center font-medium text-foreground`}>
                  ...
                  <ActionMenu
                    items={[
                      ...
                      ...(item.status === "approved" ? [
                        ...
                        ...(userRole === "doctor" ? [{ label: "Akhiri Sesi", icon: <CheckCircle2 size={14} />, onClick: () => onComplete(item.id, item.has_medical_result, item), disabled: processingId === item.id }] : []),
