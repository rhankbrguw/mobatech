...
export function CustomSnackbar({
  ...
  return (
    <div className="fixed top-4 sm:top-6 right-4 sm:right-6 left-4 sm:left-auto max-w-md z-[9999] animate-slide-in">
      <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border glass-panel backdrop-blur-xl ${bgStyles[type]}`}>
        {icons[type]}
        <span className="text-sm font-medium flex-1 break-words leading-snug">{message}</span>
        <button
          onClick={onClose}
          className="ml-4 p-1 rounded-lg hover:bg-overlay-dark dark:hover:bg-overlay-light transition-colors"
          aria-label="Close"
        >
