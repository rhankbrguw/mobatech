...
function DoctorNamePolySection({ register, errors, submitting, polyclinics, setValue, getValues }: Omit<DoctorFormFieldsProps, "control" | "doctor">) {
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      ...
      <div>
        ...
        <select disabled={submitting} name={polyName} ref={polyRef} onBlur={onPolyBlur} onChange={(e) => { onPolyChange(e); const id = Number(e.target.value); const poly = polyclinics.find((p) => p.id === id); if (poly && !getValues("specialization")) { setValue("specialization", poly.name, { shouldValidate: true }); } }} className="w-full h-10 px-3 rounded-xl border glass-input text-sm text-foreground cursor-pointer focus:border-primary outline-none">
          <option value="">Pilih Poliklinik</option>
          {polyclinics.filter((p) => p.is_active).map((p) => (<option key={p.id} value={p.id}>{p.name}</option>))}
        </select>
...
function DoctorSpecEmailSection({ register, errors, submitting, doctor }: Pick<DoctorFormFieldsProps, "register" | "errors" | "submitting" | "doctor">) {
  return (
    <div className="grid grid-cols-2 gap-4">
      <div>
      ...
      {!doctor && (
        <div>
          ...
          <input disabled={submitting} type="email" {...register("email")} className={`w-full h-10 px-3 rounded-xl border glass-input text-sm text-foreground ${errors.email ? "border-error focus:border-error" : "focus:border-primary outline-none"}`} placeholder="opsional: dr.nama@hermina.com" />
...
export function DoctorFormFields(props: DoctorFormFieldsProps) {
  return (
    <div className="space-y-4">
      <DoctorNamePolySection register={props.register} errors={props.errors} submitting={props.submitting} polyclinics={props.polyclinics} setValue={props.setValue} getValues={props.getValues} />
      <DoctorSpecEmailSection register={props.register} errors={props.errors} submitting={props.submitting} doctor={props.doctor} />
