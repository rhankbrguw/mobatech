...
export function DoctorsClient({ searchParams: _searchParams }: { initialData?: unknown, searchParams?: Record<string, string | string[] | undefined> }) {
  ...
  const raw = docsData?.data || [];
  let items = raw.map((d) => ({ ...d, id: d.id || (d as { ID?: number }).ID || 0 }));
  ...
  const saveMutation = useMutation({
    mutationFn: async (payload: Partial<Doctor> & { password?: string }) => {
      const docId = selectedItem?.id || (selectedItem as { ID?: number })?.ID;
      if (selectedItem && docId) {
        await doctorService.updateDoctor(docId, payload);
      } else {
    ...
    onError: (err: Error) => setToast({ isOpen: true, message: err.message || APP_STRINGS.login.networkError, type: "error" })
  });

  const handleSave = async (payload: { name: string; specialization: string; polyclinic_id?: number; contact_info: string; description: string; image_url: string; is_active: boolean; email?: string; password?: string }) => {
  ...
  return (
    <div className="space-y-6 animate-slide-in">
      ...
      <DoctorFormModal isOpen={showFormModal} onClose={() => setShowFormModal(false)} doctor={selectedItem} polyclinics={polyclinics} onSave={handleSave} showToast={(msg, type) => setToast({ isOpen: true, message: msg, type })} />
