  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setToast({ isOpen: true, message: data.message || data.error || "Gagal mengunggah berkas", type: "error" });
      }
    } catch {
          ...
          {uploadingFile ? (
            ...
            <>
              {fileUrl.toLowerCase().endsWith('.pdf') ? (
                ...
                <Image unoptimized={true} width={100} height={100} src={fileUrl} alt="Preview" className="w-full h-full object-cover group-hover:opacity-60 transition-opacity" />
              )}
