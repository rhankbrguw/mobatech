...
export function ImageUpload({ imageUrl, setImageUrl, label = "Upload Gambar / Foto" }: ImageUploadProps) {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      if (res.ok) {
        const urlToSet = data.data?.url || data.url;
        setImageUrl(urlToSet);
        setToast({ isOpen: true, message: APP_STRINGS.common.imageUploadSuccess, type: "success" });
      } else {
        setToast({ isOpen: true, message: data.message || data.error || "Gagal mengunggah gambar", type: "error" });
      }
    } catch {
      ...
      <div className="flex gap-4 items-start">
        ...
        >
          {uploadingImage ? (
            ...
            <>
              <Image unoptimized={true} width={100} height={100} src={imageUrl} alt="Preview" className="w-full h-full object-cover group-hover:opacity-60 transition-opacity" />
