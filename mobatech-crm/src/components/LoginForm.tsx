...
async function executeLoginSuccess(token: string, user: User, setAuth: (t: string, u: User) => void, showToast: (msg: string, type: "success"|"error"|"warning") => void) {
...
function useLoginSubmit(showToast: (msg: string, type: "success"|"error"|"warning") => void) {
  ...
  const loginMutation = useMutation({
    ...
    onSuccess: async (res) => {
      ...
      await executeLoginSuccess(res.data.token, res.data.user, setAuth, showToast);
    },
    onError: (err: unknown) => {
  ...
  return {
    ...
    onSubmit: (data: LoginFormValues) => loginMutation.mutate(data),
    onError: (errors: FieldErrors<LoginFormValues>) => handleFormValidationError(errors, showToast),
  };
}
...
function EmailField({ register, loading }: { register: UseFormRegister<LoginFormValues>; loading: boolean }) {
  return (
    <div>
      <label className="block text-xs font-semibold text-foreground/80 mb-1.5 sm:mb-2 uppercase tracking-wider">{APP_STRINGS.login.emailLabel}</label>
      <input type="email" {...register("email")} placeholder={APP_STRINGS.login.emailPlaceholder} className="w-full h-10 sm:h-11 px-3.5 sm:px-4 rounded-xl border glass-input text-xs sm:text-sm text-foreground" disabled={loading} />
    </div>
  );
}
...
function PasswordField({ register, loading, showPassword, setShowPassword }: { register: UseFormRegister<LoginFormValues>; loading: boolean; showPassword: boolean; setShowPassword: (val: boolean) => void }) {
  return (
    <div>
      <label className="block text-xs font-semibold text-foreground/80 mb-1.5 sm:mb-2 uppercase tracking-wider">{APP_STRINGS.login.passwordLabel}</label>
      <div className="relative">
        <input type={showPassword ? "text" : "password"} {...register("password")} placeholder={APP_STRINGS.login.passwordPlaceholder} className="w-full h-10 sm:h-11 pl-3.5 sm:pl-4 pr-10 rounded-xl border glass-input text-xs sm:text-sm text-foreground" disabled={loading} />
...
export function LoginForm({ showToast }: { showToast: (msg: string, type: "success"|"error"|"warning") => void }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit, onError)} className="space-y-4 sm:space-y-5">
      ...
      <button type="submit" disabled={loading} className="w-full h-10 sm:h-11 bg-primary hover:bg-primary-hover text-primary-foreground font-medium text-xs sm:text-sm rounded-xl transition-all duration-200 shadow-md flex items-center justify-center disabled:opacity-50 cursor-pointer">
