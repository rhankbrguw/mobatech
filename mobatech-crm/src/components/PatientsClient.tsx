...
export function PatientsClient({ initialData: _initialData, searchParams: _searchParams }: { initialData?: unknown, searchParams?: Record<string, string | string[] | undefined> }) {
  ...
  const { data, isLoading: loading, error } = useQuery({
    ...
    queryFn: async () => {
      const res = await adminService.getUsers(qs);
      return res;
    },
  });
  ...
  useEffect(() => {
    if (error) {
      const msg = error.message || APP_STRINGS.login.networkError;
