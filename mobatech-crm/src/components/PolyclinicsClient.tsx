...
export function PolyclinicsClient({ initialData: _initialData, searchParams: _searchParams }: { initialData?: unknown, searchParams?: Record<string, string | string[] | undefined> }) {
  const {
          ...
          <div className="space-y-3">
            <div className="flex justify-center mb-4">
              {drawerItem.image_url ? (
                <Image unoptimized={true} width={100} height={100} src={drawerItem.image_url} alt={drawerItem.name} className="w-24 h-24 rounded-xl object-cover shadow-lg border-2 border-primary-foreground/20" />
              ) : (
