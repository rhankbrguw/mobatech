...
export function RemindersClient({ initialData: _initialData, searchParams: _searchParams }: { initialData?: unknown, searchParams?: Record<string, string | string[] | undefined> }) {
  ...
  return (
    <div className="space-y-6 animate-slide-in">
      <PageHeader
        title={APP_STRINGS.reminders.pageTitle}
        description={APP_STRINGS.reminders.pageSubtitle}
        action={
          <Button onClick={() => {
            if (!showForm) {
          ...
          }} variant={showForm ? "outline" : "primary"} icon={showForm ? <X size={16} className="sm:w-[18px] sm:h-[18px]" /> : <Plus size={16} className="sm:w-[18px] sm:h-[18px]" />}>
            <span className="sm:hidden">{showForm ? APP_STRINGS.common.cancel : APP_STRINGS.common.add}</span>
            <span className="hidden sm:inline">{showForm ? APP_STRINGS.common.cancel : APP_STRINGS.reminders.addBtn}</span>
          </Button>
        }
      />
