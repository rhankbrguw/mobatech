          ...
          {selectedSchedules.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 animate-in slide-in-from-bottom-4 fade-in duration-300">
              {selectedSchedules.map((s) => {
                ...
                const _progressPercentage = Math.min((s.booked / s.quota) * 100, 100) || 0;
                  ...
                  <div key={s.id} className="group relative p-5 rounded-2xl border border-glass-border bg-overlay-dark dark:bg-overlay-light hover:bg-overlay-dark dark:hover:bg-overlay-light flex flex-col gap-4 hover:border-primary/40 hover:shadow-lg transition-all duration-300 overflow-hidden cursor-pointer">
                    ...
                    <div className="flex items-start justify-between z-10">
                      <div className="flex items-center gap-4">
                        <div className="w-11 h-11 rounded-full bg-background flex items-center justify-center overflow-hidden border border-glass-border shadow-sm group-hover:scale-105 transition-transform duration-300">
                          {s.doctor?.image_url ? (
                            <Image unoptimized={true} width={100} height={100} src={s.doctor.image_url || "/globe.svg"} alt={s.doctor.name} className="w-full h-full object-cover" />
                          ) : (
                            <span className="text-primary font-bold text-sm">{s.doctor?.name?.charAt(0) || "D"}</span>
                          )}
                        </div>
                        <div>
                          <div className="font-bold text-sm text-foreground tracking-tight line-clamp-1">{s.doctor?.name || APP_STRINGS.scheduleCalendar.defaultDoctor}</div>
                          <div className="text-xs text-foreground/60">{s.doctor?.specialization || APP_STRINGS.scheduleCalendar.defaultSpecialization}</div>
                        </div>
                      </div>
                      <Badge variant={isAvailable ? "success" : "error"} className="shadow-sm">
                        {isAvailable ? APP_STRINGS.scheduleCalendar.available : APP_STRINGS.scheduleCalendar.full}
                      </Badge>
                    </div>
                    ...
                    <div className="z-10 w-full space-y-1.5 mt-auto">
                      <div className="flex justify-between items-end text-xs">
                        <span className="text-foreground/70 font-medium">{APP_STRINGS.scheduleCalendar.quotaAvailable}: <span className="text-foreground font-bold">{s.quota - s.booked}</span></span>
                        <span className="text-foreground/50 text-[10px]">{APP_STRINGS.scheduleCalendar.quotaFilled} {s.booked}/{s.quota}</span>
                      </div>
                      <div className="h-1.5 w-full bg-overlay-dark dark:bg-overlay-light rounded-full overflow-hidden">
                        <div 
                          className={`h-full rounded-full transition-all duration-1000 ${isAvailable ? 'bg-primary' : 'bg-destructive'} w-full`}
                        ></div>
                      </div>
                    </div>
                  </div>
