...
export const FilterDropdown: React.FC<FilterDropdownProps> = ({ 
  ...
  return (
    <div className={`relative flex items-center w-full sm:max-w-[260px] rounded-xl border border-black/5 dark:border-white/10 bg-surface-primary/70 dark:[background-color:var(--color-dark-panel)] backdrop-blur-[12px] saturate-[180%] shadow-sm transition-all focus-within:ring-2 focus-within:ring-primary focus-within:border-primary/50 ${className}`}>
      ...
      >
        {!options.some(opt => opt.value === "") && (
          <option value="" disabled hidden className="bg-background text-foreground">{placeholder}</option>
        )}
        {options.map((opt) => (
