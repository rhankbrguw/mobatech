...
export const adminService = {
  ...
  actionAppointment: (id: number | string, action: string, data?: { reason?: string }) =>
    api.post(`/api/admin/appointments/${id}/${action}`, data || {}),
...
export const pharmacistService = {
  getPharmacists: (qs: string = "") => {
    const sep = qs.includes("?") ? "&" : "?";
    return api.get<User[]>(`/api/admin/users${qs}${sep}role=pharmacist`);
  },
  createPharmacist: (data: { full_name: string; email: string; phone_number: string; password?: string; image_url?: string; role: string }) =>
    api.post<User>("/api/admin/users", { ...data, role: "pharmacist" }),
  updatePharmacist: (id: number | string, data: Partial<User> & { password?: string }) =>
    api.put<User>(`/api/admin/users/${id}`, { ...data, role: "pharmacist" }),
  deletePharmacist: (id: number | string) => api.delete(`/api/admin/users/${id}`),
};

export const notificationService = {
  getNotifications: (unread?: boolean, page: number = 1, limit: number = 20) => {
    const q = new URLSearchParams({ page: String(page), limit: String(limit) });
    if (unread) q.append("unread", "true");
    return api.get<{ notifications: AppNotification[]; total: number; unread_count: number; page?: number; limit?: number }>(
