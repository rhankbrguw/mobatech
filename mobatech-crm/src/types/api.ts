...
export interface FamilyMember extends GormModel {
  user_id: number; name: string; relation: string; date_of_birth: string;
}
export interface User extends GormModel {
  ...
  gender?: string; family_members?: FamilyMember[];
}
...
export interface Doctor extends GormModel {
  ...
  is_active: boolean; is_available_today?: boolean;
}
...
export interface Prescription extends GormModel {
  user_id: number; appointment_id: number;
  doctor_name: string; diagnosis: string;
  status: string; items: PrescriptionItem[];
}

export interface MedicalResult extends GormModel {
  user_id: number; appointment_id?: number;
  doctor_name: string; test_type: string;
  test_name: string; result: string; notes: string;
  file_url: string; result_date: string; has_prescription?: boolean;
}

export interface AppNotification extends GormModel {
  user_id?: number; role: string;
  title: string; message: string;
  type: "emergency" | "appointment" | "prescription" | "general" | string;
  action_url?: string; is_read: boolean;
}
